...
class ExtensionsControllerTest extends BootstrapTestSuite
{
    ...
    public function testExtensionsGeneral()
    {
        ...
        // Content Check
        $this->assertTrue($crawler->filter('html:contains("The following policies should be followed when developing extensions for phpBB")')->count() > 0, 'Extensions Home Content Check');
        $this->assertTrue($crawler->filter('html:contains("Extensions Team")')->count() > 0, 'Extensions Sidebar Check');
    ...
    public function testExtensionsInstaDeny()
    {
        ...
        // Content Check
        $this->assertTrue($crawler->filter('html:contains("Insta-Deny is meant to detect basic problems in an extension")')->count() > 0, 'Extensions Home Content Check');
        $this->assertTrue($crawler->filter('html:contains("Extensions Team")')->count() > 0, 'Extensions Sidebar Check');
    ...
    public function testExtensionsRepack()
    {
        ...
        // Content Check
        $this->assertTrue($crawler->filter('html:contains("All extensions submitted to the Extensions Database can be repacked")')->count() > 0, 'Extensions Home Content Check');
        $this->assertTrue($crawler->filter('html:contains("Extensions Team")')->count() > 0, 'Extensions Sidebar Check');
